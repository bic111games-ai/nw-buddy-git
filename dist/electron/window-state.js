...
function isWithinBounds(bounds, state) {
    return (!!state &&
        !!bounds &&
        state.x >= bounds.x &&
        state.y >= bounds.y &&
...
function isStateValid(state) {
    ...
    const display = electron_1.screen.getAllDisplays().find((it) => isWithinBounds(it.bounds, state));
    return !!display;
}
function createDefaultState(bounds) {
    return Object.assign({ width: 800, height: 600, x: 0, y: 0, displayBounds: electron_1.screen.getPrimaryDisplay().bounds }, (bounds || {}));
}
function windowState(options) {
    let state = {};
    let winRef;
    ...
    state = Object.assign({ width: config.defaultWidth || 800, height: config.defaultHeight || 600 }, (state || {}));
    function resetState() {
    ...
    function updateState(win = winRef) {
        if (!win) {
            return;
        }
        ...
        try {
            ...
            state.displayBounds = electron_1.screen.getDisplayMatching(bounds).bounds;
        }
        catch (err) { }
    }
